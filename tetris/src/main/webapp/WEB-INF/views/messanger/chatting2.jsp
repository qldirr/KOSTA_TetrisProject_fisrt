		...
		<c:forEach items="${listChatContents }" var="listCC">
			<c:choose>
				<c:when test="${listCC.cc_file eq false }">
					<c:out value="${listCC.userVO.userName }"/>: <c:out value="${listCC.cc_contents }"/>  <c:out value="${listCC.cc_regdate }"/><br>
				</c:when>
				<c:otherwise>
					<c:choose>
	...
	<div id="chat-body">
            ...
            <main>
                ...
                <div class="insert-content">
					<textarea name="chat-insert" id="message" required></textarea>
...
<script type="text/javascript">
	...
	function sendMessage() {
		...
		$(".me-chat .balloon").append('${principal.user.userName}' + ": " + $("#message").val() + "<br/>");
	}
	// 서버로부터 메시지를 받았을 때
	function onMessage(msg) {
		
		var data = msg.data;
		$(".friend-chat .balloon").append(data + "<br/>");
